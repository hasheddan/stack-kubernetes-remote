...
const (
	...
	reasonCannotApplyWorkloadPackge = "CannotApplyWorkloadPackage"
)
...
func NewReconciler(m ctrl.Manager, workload Kind, o ...ReconcilerOption) *Reconciler {
	...
	r := &Reconciler{
		client:      m.GetClient(),
		newWorkload: nw,
		packager:    PackageFn(genericPackager),
...
func (r *Reconciler) Reconcile(req reconcile.Request) (reconcile.Result, error) {
	...
	if err := r.client.Get(ctx, req.NamespacedName, app); resource.IgnoreNotFound(err) != nil {
		log.Debug("Cannot get workload package", "error", err, "requeue-after", time.Now().Add(shortWait))
		r.record.Event(workload, event.Warning(reasonCannotGetPackage, err))
		workload.SetConditions(v1alpha1.ReconcileError(errors.Wrap(err, errGetPackage)))
		return reconcile.Result{RequeueAfter: shortWait}, errors.Wrap(r.client.Status().Update(ctx, workload), errUpdateWorkloadStatus)
	}

	if err := r.packager.Package(ctx, app, workload); err != nil {
	...
	app.SetOwnerReferences(workload.GetOwnerReferences())
	app.SetName(workload.GetName())
	app.SetNamespace(workload.GetNamespace())
	if err := resource.Apply(ctx, r.client, app, resource.ControllersMustMatch()); err != nil {
		...
		r.record.Event(workload, event.Warning(reasonCannotApplyWorkloadPackge, err))
